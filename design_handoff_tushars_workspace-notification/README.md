...
claude-queue/
  ...
  static/
    ...
    css/notifications.css ← src/css/notifications.css (toasts + notification centre)
    ...
    js/notifications.js   ← src/js/notifications.js (NotificationCenter: toasts, bell, panel)
    js/types.js           ← src/js/types.js      (JSDoc typedefs)
    js/views/             write these:
      shell.js            topbar (+ bell), sidebar (desk), tab bar + FAB (phone), pager, density, mounts NotificationCenter
      splash.js
...
### Toasts & notification centre
Code: `src/js/notifications.js` (`NotificationCenter`) + `src/css/notifications.css`. Reference: search `notifVals` / `mkNotif` in the prototype.

**Toasts**
- **Placement:** top-right, just under the topbar. Desktop `top 78px, right 24px, width 380px`; phone `top calc(70px + safe-top)`, 12px sides (full width). Stack of up to 3, 8px gap, **newest at the bottom**; the oldest drops when a 4th arrives.
- **Card:** 18px radius, `rgba(18,25,34,.94)` + 16px blur, 1px `--line-strong`, shadow `0 18px 44px rgba(0,0,0,.55)` + 1px top inner highlight. Grid: 38px kind icon tile (radius 12, kind colour at 14%) · kicker (10.5px mono 600, .08em, kind colour) / title (14.5px 600, one line) / sub (11.5px mono meta, one line) · optional action button (36px, `--surface-2`, kind-coloured text) + ✕ (36px ghost).
- **Countdown:** 2px kind-coloured bar along the bottom edge shrinks over the toast's life: 5s, or 6s when it has an action (Undo).
- **Motion:** `toast-in .42s cubic-bezier(.2,.8,.25,1)`: slides in 32px from the right, scale .96 → 1.
- **Tap:** opens the related task (chat, or the approval pane when it's waiting). ✕ dismisses. The action runs, then dismisses.
- **Copy → kind:** `parseToast()` splits the existing strings on " · ". A leading status word becomes the kicker ("Done · Fix RSS · $0.27" → DONE / Fix RSS / $0.27; "→ GPT-6 Luna · answering · #12" → SENT / GPT-6 Luna / answering · #12). All the existing messages keep working unchanged.

**Kinds** (`KINDS`): done `#6FCF97` check · answer `#8FE0B0` bubble · approval `#FFB547` clock · approved `#FFB547` check · stop `#FF8FA2` ✕ · queue `#FF7B5C` list-arrow · error `#FF5470` triangle · review `#7FB2FF` eye · limit `#A58BFF` moon · add `#6FCF97` plus · polish and info are **ephemeral** (toast only, never listed).

**Bell** (topbar, between the usage pill and the density toggle): 40px `.icon-btn`; `is-open` state uses `--surface-2` / `--line-btn`. Unread badge: 16px coral pill with a 2px `--bg` ring, top-right −6px, "9+" cap. On phone the "live" dot is hidden to make room.

**Panel**
- Top-right popover under the topbar, desktop 400px wide (max `min(640px, 100% − 110px)`), phone 12px from both edges. 22px radius, `--bg-sheet`, `--sh-modal`, `notif-in .24s` from top-right. Scrim: desktop `rgba(3,5,8,.28)`, phone `.62`.
- **Header:** "Notifications" 20px 800 · coral "N new" pill · "Mark all read" (phone: "Read all") · ✕.
- **Tabs:** segmented All / Needs you with mono counts; the Needs-you count is amber.
- **Rows:** 38px icon tile · kicker + "12m ago" · title (14px 600, 2-line clamp) · sub · CTA chip (**Review request** / **See what broke** / **Review changes**) only while that task is still waiting · 8px coral unread dot. Unread rows get a `rgba(255,123,92,.05)` wash.
- **Groups:** All → NEW, EARLIER. Needs you → WAITING ON YOU.
- **Empty:** sleeping Octopus 64px + "All caught up" / "Nothing waiting on you".
- **Footer:** "N notifications" · **Clear read** (keeps anything still waiting).
- **Behaviour:** opening hides live toasts and suppresses new ones, which still land in the list. Closing (✕, scrim, Esc) marks everything read. Tapping a row opens the task and closes the panel.

**Source of notifications:** derived client-side in `ingest(store.data)` after every poll: new pending approvals, and tasks moving to done / error / needs_review / rate_limited. The first poll backfills silently (anything still waiting, plus the last 24h; items older than 1h are marked read). Deduped by key and persisted in `localStorage['octopus-notifs']` (max 60). UI confirmations go through `notify.toast(text, { action, actionLabel, taskId })`.
...
## State management
`api.js` exports `store.data` (server snapshot) and `store.ui`: view, projectId, projectTab, density, skeleton, pane, sheet, projectsList{quick, sort, q, searchOpen, menu, showPaused, onlyActive, open}, ask{text, model, projectId, images, sending, result, menu}, activityFilter, booted.
...
- `src/css/tokens.css`, `src/css/components.css`, `src/css/notifications.css`: production CSS.
- `src/js/api.js`, `format.js`, `octopus.js`, `gestures.js`, `polish.js`, `icons.js`, `notifications.js`, `types.js`: production helpers (ES modules, no build).

- `NOTIFICATIONS_PROMPT.md`: the Claude Code prompt for the toast + notification centre update.
